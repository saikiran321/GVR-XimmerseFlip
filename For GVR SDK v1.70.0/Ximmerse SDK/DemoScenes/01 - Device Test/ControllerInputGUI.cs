...
public class ControllerInputGUI:MonoBehaviour {
	...
	public class ControllerEntry{
		...
		public void Update(){
            ...
            // 重置状态
            if(controllerInput.connectionState!=DeviceConnectionState.Connected)
            {
                ResetValue();
                return;
            }
        ...
        private void ResetValue()
        {
            int i,imax;
			for(i=0,imax=m_Buttons.Length;i<imax;++i){
				m_Buttons[i].ResetValue();
			}
			if(m_Rotation!=null) {
				m_Rotation.value=Vector3.zero;
			}
			if(m_Accelerometer!=null) {
				m_Accelerometer.value=Vector3.zero;
			}
			if(m_Gyroscope!=null) {
				m_Gyroscope.value=Vector3.zero;
			}
			if(m_AccelerometerMag.image!=null) {
				m_AccelerometerMag.Update(0);
			}
			if(m_GyroscopeMag.image!=null) {
				m_GyroscopeMag.Update(0);
			}
        }
